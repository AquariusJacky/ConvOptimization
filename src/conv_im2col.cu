...
__global__ void conv_im2col_forward(const float* input,   // [N, C, H, W]
                                    const float* kernel,  // [K, C, R, S]
                                    float* output,  // [N, K, H_out, W_out]
                                    size_t N, size_t C, size_t H, size_t W,
                                    size_t K, size_t R, size_t S, size_t pad,
                                    size_t stride, size_t H_out, size_t W_out) {
  __shared__ half tile_kernel[TILE_SIZE][WMMA_K];
  __shared__ half tile_im2col[WMMA_K][TILE_SIZE];
  __shared__ float tile_output[TILE_SIZE][TILE_SIZE];
...
#pragma unroll
    for (int idx = threadIdx.x; idx < (TILE_SIZE * WMMA_K); idx += BLOCK_SIZE) {
      ...
      if (k_out < K && crs_idx < CRS) {
        tile_kernel[tile_row][tile_col] =
            __float2half(kernel[k_out * CRS + crs_idx]);
      } else {
...
void conv_forward(const float* input, const float* kernel, float* output,
  ...
  // 4 warps because each block computes a 64x64 tile
  // and each warp computes a 32x32 tile
  // that is 4 wmma calls per warp
